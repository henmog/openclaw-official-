...
# Keep npm as an operator-facing capability while replacing the base image's CLI tree
...
# Create startup entrypoint with automatic device pairing background approver
RUN printf '#!/bin/sh\n\
...
if [ "$#" -gt 0 ]; then\n\
  ...
  exec node /app/openclaw.mjs gateway run --allow-unconfigured --bind lan --port 10000\n\
fi\n' > /app/entrypoint.sh && \
...
ENTRYPOINT ["tini", "-s", "--", "/app/entrypoint.sh"]
CMD []
