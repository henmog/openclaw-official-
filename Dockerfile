...
# Pre-configure trustedProxies so Render's proxy is accepted
RUN echo '{"gateway":{"mode":"local","bind":"lan","port":10000,"trustedProxies":["127.0.0.1","::1"],"controlUi":{"dangerouslyAllowHostHeaderOriginFallback":true}}}' > /home/node/.openclaw/openclaw.json && \
    chown -R node:node /home/node/.openclaw

# Create the startup script with background auto-approval
RUN printf '#!/bin/bash\n\
(while true; do\n\
  node /app/openclaw.mjs devices approve --latest >/dev/null 2>&1 || true\n\
  sleep 2\n\
done) &\n\
exec node /app/openclaw.mjs gateway run --port 10000 --bind lan --allow-unconfigured\n' > /app/entrypoint.sh && \
