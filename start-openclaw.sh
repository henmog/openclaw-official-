...
# 1. Set Token, Password, and Paths
...
echo "===================================================="
echo "🔑 YOUR OPENCLAW CREDENTIAL IS:"
echo "$TOKEN"
echo "===================================================="
...
# 3. Write gateway configuration with both token and password
...
# 4. Background daemon to auto-approve device pairing with password
(
  echo "=== Device auto-approval daemon started ==="
  sleep 20

  # Print clean dashboard link
  echo "=================================================================="
  echo "🔗 DIRECT DASHBOARD LINK:"
  openclaw dashboard --no-open 2>&1 || true
  echo "=================================================================="

  # Auto-approve loop using configured password
  while true; do
    openclaw devices approve --latest --url ws://127.0.0.1:10000 --password "$TOKEN" >/dev/null 2>&1 || true
    openclaw devices approve --latest --password "$TOKEN" >/dev/null 2>&1 || true
    sleep 3
  done
) &
